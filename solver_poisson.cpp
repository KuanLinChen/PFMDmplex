...
void CPoisson::Bulid_A_B()
{
  ...
  for ( unsigned int i = 0 ; i < m->interior_face_loop.size() ; i++ ) {
		Ad_dPN= m->interior_face_loop[i]->dA / m->interior_face_loop[i]->dPN ;
		//C0
		P = m->interior_face_loop[i]->gc0 ;
		N = m->interior_face_loop[i]->gc1 ;
		s->Add_Entries( P, P, -Ad_dPN ) ;
		s->Add_Entries( P, N,  Ad_dPN ) ;
		//C1
  	P = m->interior_face_loop[i]->gc1 ;
  	N = m->interior_face_loop[i]->gc0 ;
  	s->Add_Entries( P, P, -Ad_dPN ) ;
		s->Add_Entries( P, N,  Ad_dPN ) ;
	}

	for ( unsigned int i = 0 ; i < m->processors_face_loop.size() ; i++ ) {
		Ad_dPN= m->processors_face_loop[i]->dA / m->processors_face_loop[i]->dPN ;
		//C0
		P = m->processors_face_loop[i]->gc0 ;
		N = m->processors_face_loop[i]->gc1 ;
		s->Add_Entries( P, P, -Ad_dPN ) ;
		s->Add_Entries( P, N,  Ad_dPN ) ;
	}

	for ( unsigned int i = 0 ; i < m->ground_face_loop.size() ; i++ ) {
		Ad_dPN =  m->ground_face_loop[i]->dA /  m->ground_face_loop[i]->dPN ;
   	P =  m->ground_face_loop[i]->gc0 ;
   	s->Add_Entries( P, P, -Ad_dPN ) ;
	}

	double BCValue = 10.0 ;
	for ( unsigned int i = 0 ; i < m->power_face_loop.size() ; i++ ) {
		Ad_dPN=  m->power_face_loop[i]->dA /  m->power_face_loop[i]->dPN ;
  	P =  m->power_face_loop[i]->gc0 ;
...
void CPoisson::CalculateGraditntLSQ()
{
	...
	for ( unsigned int i = 0 ; i < m->neumann_face_loop.size() ; i++ ) {
  	...
  	var->FaceField[ ids ] = value[ m->neumann_face_loop[i]->gc0 ];
	}
	...
	for ( int i = m->cStart ; i < m->cEndInterior ; i++ ) {
		...
		for ( unsigned int k = 0 ; k < m->cell_map_all[i]->nghbr_cell.size() ; k++ ) {
			...
			Gx[ i ] = Gx[ i ] +  m->cell_map_all[i]->nghbr_cell_Cx[ k ]*dVar ;
			Gy[ i ] = Gy[ i ] +  m->cell_map_all[i]->nghbr_cell_Cy[ k ]*dVar ;
		}
		for ( unsigned int k = 0 ; k < m->cell_map_all[i]->nghbr_face.size() ; k++ ) {

			ids = m->cell_map_all[i]->nghbr_face[ k ]-m->fStart ;
			dVar =  var->FaceField[ids] - value[ i ];
			Gx[ i ] = Gx[ i ] +  m->cell_map_all[i]->nghbr_face_Cx[ k ]*dVar ;
			Gy[ i ] = Gy[ i ] +  m->cell_map_all[i]->nghbr_face_Cy[ k ]*dVar ;
